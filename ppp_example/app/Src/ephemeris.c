...
static double var_urassr(int ura)
{
    ...
    double temp[63]={0};
    for(int i=1;i<63;i++){
        temp[i]=(pow(3.0,(i>>3)&7)*(1.0+(i&7)/4.0)-1.0)*1E-3;
        temp[i]=SQR(temp[i]);
    }
...
*          (tgd or bgd)
*-----------------------------------------------------------------------------*/
extern void eph2ek(gtime_t time,const eph_t *eph,double *relA,double *ek){
    int n ,sys,prn;
    double mu,omge;
    double bdsA0;
    double tk;
    double M, E,Ek;
    if(eph->sat<=0||eph->sat>MAXSAT){
        return;
    }
    ...
    bdsA0 = eph->A;
    if(sys==SYS_CMP&&prn>=19&&prn<=63){
        if((prn>=19&&prn<=37)||(prn>=41&&prn<=46)){
            if(eph->delA) bdsA0= A_REF_MEO+eph->delA;
        }
        else if((prn>=38&&prn<=40)||(prn>=59&&prn<=61)){
            if(eph->delA) bdsA0= A_REF_IGSO_GEO+eph->delA;
        }
        else {
            bdsA0 = 0;
        }
    ...
    M=eph->M0+(sqrt(mu/(bdsA0*bdsA0*bdsA0))+eph->deln+0.5*eph->ndot*tk)*tk;
    bdsA0 +=eph->Adot*tk;
                    ...
                    double *var)
{
    ...
    double xg,yg,zg,sino,coso,bdsA0=0;
    int n,sys,prn;
    double n0=0;
    double nA=0;
    double deltN=0;
    ...
    n0=sqrt(mu/(bdsA0*bdsA0*bdsA0));
    deltN=eph->deln+0.5*eph->ndot*tk;
    ...
    *dts=eph->f0+eph->f1*tk+eph->f2*tk*tk;
    ...
    //*dts-=2.0*sqrt(mu*bdsA0)*eph->e*sinE/SQR(CLIGHT);
...
/* select ephememeris --------------------------------------------------------*/
static eph_t *seleph(gtime_t time, int sat, int iode, int bdsephsource,const nav_t *nav)
{
    double t,tmax,tmin;
    int i,j=-1,sys,sel;
    double tempIOde;
    double tempEp[6]={2021,11,03,16,01,30};
    if(timediff(time,epoch2time(tempEp))>=0&&timediff(time,epoch2time(tempEp))<=29){
        tempEp[0]=2021;
    }
    ...
    for (i=0;i<nav->n;i++) {
        ...
        if (sys==SYS_CMP){
            tempIOde = nav->eph[i].datasource;
            if(bdsephsource>=0&&nav->eph[i].datasource!=bdsephsource) continue;
        }
    ...
    /*if (iode>=0||j<0) {
        trace(3,"seleph:no broadcast ephemeris: %s sat=%2d iode=%3d\n",
                  ...
                  double *dts)
{
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP||sys==SYS_IRN) {
        if (!(eph=seleph(teph,sat,ssr.iode,-1,nav))) return 0;
...
static int ephpos(gtime_t time, gtime_t teph, int sat, int bdsephsource,const nav_t *nav,
                  int iode, double *rs, double *dts, double *var, int *svh)
{
    ...
    *svh=-1;
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP||sys==SYS_IRN) {
        if (!(eph=seleph(teph,sat,iode,bdsephsource,nav))) return 0;
    ...
    if (!normv3(rc,ec)) {
        *svh=-1;
        memset(rs,0,6*sizeof(double));
        return 0;
    }
    cross3(ea,ec,er);
    //trace(1,"#Brd: %s,%3d,%10.10f,%10.10f,%10.10f,%6.10f,%6.10f,%6.10f,%6.10f,%6.10f,%6.10f,%6.10f,%6.10f,%6.10f,%6.10f,\n",
      //time_str(time,2),sat,rs[0],rs[1],rs[2],dts[0],er[0],er[1],er[2],ea[0],ea[1],ea[2],ec[0],ec[1],ec[2]);
    
    return 1;
}
/* satellite position and clock with sbas correction -------------------------*/
static int satpos_sbas(gtime_t time, gtime_t teph, int sat,int bdsephsource, const nav_t *nav,
                        double *rs, double *dts, double *var, int *svh)
{
    ...
    if (i>=nav->sbssat.nsat) {
        ...
        ephpos(time,teph,sat,bdsephsource,nav,-1,rs,dts,var,svh);
        *svh=-1;
        return 0;
    }
    /* satellite postion and clock by broadcast ephemeris */
    if (!ephpos(time,teph,sat,nav,bdsephsource,sbs->lcorr.iode,rs,dts,var,svh)) return 0;
...
static int satpos_ssr(gtime_t time, gtime_t teph, int sat, int bdsephsource,const nav_t *nav,
                      int opt, double *rs, double *dts, double *var, int *svh)
{
    const ssr_t *ssr;
    eph_t *eph;
    double temp;
    double t1,t2,t3,t4,et[3],ew[3],en[3],rc[3],deph[3],dclk,dant[3]={0},tk;
    int i,sys;
    double tempR[6] ={0};
    double tempClk  =0;
    char id[4];
    ...
    time2epoch(teph,tempTime);
    trace(4,"satpos_ssr: time=%s sat=%2d\n",time_str(time,3),sat);
    
    ssr=nav->ssr+sat-1;
    
    if(tempTime[3]==9&&tempTime[4]==23){
        tempTime[3]=tempTime[3];
    }
    if (!ssr->t0[0].time) {
    ...
    if (!ssr->t0[2].time){
        trace(2,"no ssr code bias correction: %s sat=%2d\n",time_str(time,0),sat);
        memset(rs,0,6*sizeof(double));
        return 0;
    }
    ...
    /* satellite postion and clock by broadcast ephemeris */
    if (!ephpos(time,teph,sat,bdsephsource,nav,ssr->iode,rs,dts,var,svh)) return 0;
    //trace(1,"b1c: %s sat=%2d deph=%10.5f %10.5f %10.5f\n",
        //  time_str(time,2),sat,rs[0],rs[1],rs[2]);

    /* satellite clock for gps, galileo and qzss */
    sys=satsys(sat,NULL);
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP) {
        if (!(eph=seleph(teph,sat,ssr->iode,bdsephsource,nav))) return 0;
        ...
        /* relativity correction */
        eph2ek(time,eph,&relA,&E);
        temp = -4.442807309e-10*eph->e*sqrt(relA)*sin(E);
//        dts[0] += -2*sqrt(3.986004418e14)/(CLIGHT*CLIGHT)*eph->e*sqrt(relA)*sin(E);
        dts[0] += -2.0*dot(rs,rs+3,3)/CLIGHT/CLIGHT;
    }
    /* radial-along-cross directions in ecef */
    memcpy(tempR,rs,6*sizeof(double));
    if (!normv3(rs+3,et)) return 0;
    cross3(rs,rs+3,rc);
    if (!normv3(rc,ew)) {
        *svh=-1;
        memset(rs,0,6*sizeof(double));
...
//        satantoff(time,rs,sat,nav,dant);
    ...
    //}
    for (i=0;i<3;i++) {
        rs[i]+=(en[i]*deph[0]+et[i]*deph[1]+ew[i]*deph[2])+dant[i];
    }
...
//    tempCbia=(SQR(FREQ1_CMP)/(SQR(FREQ1_CMP)-SQR(FREQ3_CMP)))*nav->ssr[sat-1].cbias[39];
    outCLk=dts[0]*CLIGHT;
//    trace(1,"#HAS: %s,%3d,%6.10f,%6.10f,%10.10f,%10.10f,%10.10f,%10.10f,%10.10f,%10.10f,%10.10f,%10.10f,%10.10f\n",
//      time_str(time,2),sat,tempCLk,outCLk,rs[0],rs[1],rs[2],rs[3],rs[4],rs[5],tempR[0],tempR[1],tempR[2]);
    //trace(1,"sat = %d tempR : x=%10.6f y=%10.6f z=%10.6f  tempClk =%10.6f  vx =%10.6f vy =%10.6f vz =%10.6f\n",sat,tempR[0],tempR[1],tempR[2],tempClk,tempR[3],tempR[4],tempR[5]);
...
*-----------------------------------------------------------------------------*/
extern int satpos(gtime_t time, gtime_t teph, int sat, int ephopt,int bdsephsource,
                  ...
                  int *svh)
{
    ...
    *svh=0;
    double temp=0;
    if(teph.time==1648619250&&sat==120){
        temp=1;
        //time.time=1648619249;
        //time.sec=0.92339398044;
    }
    
    switch (ephopt) {
        case EPHOPT_BRDC  : return ephpos     (time,teph,sat,bdsephsource ,nav,-1,rs,dts,var,svh);
        case EPHOPT_SBAS  : return satpos_sbas(time,teph,sat,bdsephsource ,nav,   rs,dts,var,svh);
        case EPHOPT_SSRAPC: return satpos_ssr (time,teph,sat,bdsephsource ,nav, 0,rs,dts,var,svh);
        case EPHOPT_SSRCOM: return satpos_ssr (time,teph,sat,bdsephsource ,nav, 1,rs,dts,var,svh);
        case EPHOPT_PREC  :
                    ...
                    const prcopt_t *opt,double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        if (!satpos(time[i],teph,obs[i].sat,opt->sateph,opt->bdsephsource,nav,rs+i*6,dts+i*2,var+i,
                    svh+i)) {
