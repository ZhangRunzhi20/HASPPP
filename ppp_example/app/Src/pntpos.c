...
#endif
#define MAXITR      20          /* max number of iteration for point pos */
#define ERR_ION     0.03         /* ionospheric delay Std (m) */
#define ERR_TROP    0.3        /* tropspheric delay Std (m) */
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* dual-frequency */
        ...
        *var=SQR(ERR_CBIAS);
        ...
        if (sys==SYS_GPS||sys==SYS_QZS) { /* L1 */
            ...
            b1 = nav->ssr[sat-1].cbias[obs->code[0]-1];
            //if(b1 == 0) b1=gettgd(sat,nav,0);
            //if      (obs->code[0]==CODE_L2I) b1=gettgd(sat,nav,0); /* TGD_B1I */
            //else if (obs->code[0]==CODE_L1P) b1=gettgd(sat,nav,2); /* TGD_B1Cp */
            //else b1=gettgd(sat,nav,2)+gettgd(sat,nav,4); /* TGD_B1Cp+ISC_B1Cd */
            return P1;
                   ...
                   double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        //var[nv++]=2*varerr(opt,azel[1+i*2],sys)+vare[i]+vmeas+vion+vtrp;
        var[nv++]=10*varerr(opt,azel[1+i*2],sys);
        tempVar[0]=varerr(opt,azel[1+i*2],sys);
        tempVar[1]=vare[i];
        tempVar[2]=vmeas;
        tempVar[3]=vion;
        tempVar[4]=vtrp;
        //debug by zhangrunzhi
        //var[nv++]=10*varerr(opt,azel[1+i*2],sys)+vmeas+vion+vtrp;
