...
#define MAX_ITER    18               /* max number of iterations */
                      ...
                      double *Lc, double *Pc)
{
    ...
    for (i=0;i<NFREQ;i++) {
        ...
        if(opt->sateph == EPHOPT_SSRAPC||opt->sateph == EPHOPT_SSRCOM){
            if(sys==SYS_GPS){
                subscript = obs->code[i]-1;
                if(subscript>=0&&nav->ssr[obs->sat-1].cbias[subscript]!=0){
                    P[i]+=nav->ssr[obs->sat-1].cbias[subscript];
                }
                   ...
                   double *azel)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        if ((r=geodist(rs+i*6,rr,e))<=0.0||
            ...
            if(r>0)
                trace(4,"%s sat=%2d el=%4.1f\n",str,sat,azel[1+i*2]*R2D);
            exc[i]=1;
            continue;
        }
        if (nav->ssr[sat-1].cbias[obs[i].code[0]-1]==0||nav->ssr[sat-1].cbias[obs[i].code[1]-1]==0){
            trace(2,"%d no dcb!\n",sat);
            exc[i]=1;
            continue;
        }
        ...
        for (j=0;j<2*NF(opt);j++) {
            ...
            /* variance */
            var[nv]=1*varerr(obs[i].sat,sys,azel[1+i*2],j/2,j%2,opt)+
                    vart+SQR(C)*vari+var_rs[i];
            //var[nv]=10*varerr(obs[i].sat,sys,azel[1+i*2],j/2,j%2,opt)+
               //     var_rs[i];
            //var[nv]=0.009*varerr(obs[i].sat,sys,azel[1+i*2],j/2,j%2,opt);
            var[nv]=0.1*varerr(obs[i].sat,sys,azel[1+i*2],j/2,j%2,opt);
            ...
            /*if (post&&opt->maxinno>0.0&&fabs(v[nv])>0.06&&j%2==0) {
                trace(2,"outlier (%d) rejected %s sat=%2d %s%d res=%9.4f el=%4.1f\n",
                      post,str,sat,j%2?"P":"L",j/2+1,v[nv],azel[1+i*2]*R2D);
                exc[i]=1; rtk->ssat[sat-1].rejc[j%2]++;
                stat=0;
            ...
            if (post&&opt->maxinno>0.0&&fabs(v[nv])>6&&j%2==1) {
                trace(2,"outlier (%d) rejected %s sat=%2d %s%d res=%9.4f el=%4.1f\n",
                      post,str,sat,j%2?"P":"L",j/2+1,v[nv],azel[1+i*2]*R2D);
                exc[i]=1; rtk->ssat[sat-1].rejc[j%2]++;
                stat=0;
...
extern void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    /* exclude measurements of eclipsingR satellite (block IIA) */
    if (rtk->opt.posopt[3]) {
    ...
    for (i=0;i<MAX_ITER;i++) {
        ...
        memcpy(temprs,rs,6*n*sizeof(double));
        /* prefit residuals */
        ...
        if (ppp_res(i+1,obs,n,rs,dts,var,svh,dr,exc,nav,xp,rtk,v,H,R,azel)) {
            ...
            memcpy(tempClk,rtk->x,9*sizeof(double));
