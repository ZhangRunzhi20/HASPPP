                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    for (i=0;i<NFREQ;i++) {
        ...
        for (j=0;j<n;j++) {
            if (ind->code[j]==ind->code[k]) ind->pos[j]=i;/*sign max*/
        }
    }
                      ...
                      eph_t *eph)
{
    ...
    gtime_t timeTemp={0};
    double tempSat = 0;
    double tempEP[6]={2021,11,03,14,0,0};
    int i = 0;
...
//        eph->code=eph->code&7;
        ...
        timeTemp = epoch2time(tempEP);
        if(data[11] == 306000 && sat == 119){
            tempSat =191;
        }
        ...
        /* IGMAS */
        bdsat = sat - NSATGPS -NSATGLO -NSATGAL -NSATQZS;
        if(bdsat>=19&&bdsat<=63){
            eph->datasource = (int)data[20];
            if(eph->datasource>=BDS_EPH_B1C && eph->datasource<= BDS_EPH_B2a){
                eph->delA =data[10];
                eph->Adot =data[22];
                eph->ndot =data[29];
                eph->tgd[2]=data[30];
                inter=(int)data[23];
                setbitu(buff,0,31,inter);
                eph->SISMAI  =getbits(buff,i,4);i+=4;
                eph->SISAIOE =getbits(buff,i,5);i+=5;
                eph->top     =getbits(buff,i,11);i+11;
                eph->SISAocb =getbits(buff,i,5);i+=5;
                eph->SISAoc1 =getbits(buff,i,3);i+=3;
                eph->SISAoc2 =getbits(buff,i,3);i+=3;
            }
