...
static void outsolstat(rtk_t *rtk)
{
    ...
    char timestr[1024];
    time2str(rtk->sol.time,timestr,3);
...
//                    ssat->lock[j],ssat->outc[j],ssat->slipc[j],ssat->rejc[j]);
            fprintf(fp_stat,"$SAT,%s,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.1f,%d,%d,%d,%d,%d,%d\n",
                    timestr,id,j+1,ssat->azel[0]*R2D,ssat->azel[1]*R2D,
                    ssat->resp[j],ssat->resc[j],ssat->vsat[j],
                    ssat->snr[j]*SNR_UNIT,ssat->fix[j],ssat->slip[j]&3,
                    ssat->lock[j],ssat->outc[j],ssat->slipc[j],ssat->rejc[j]);
        }
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    int i,nu,nr;
    char msg[128]="";
    double tempPos[3]={30.515567*D2R,114.494015*D2R,72.5};
    double tempEcef[3]={0};
    ...
    if(opt->mode == PMODE_PPP_STATIC ||opt->mode ==PMODE_PPP_KINEMA){
        for(i=0;i<3;i++){
            rtk->rb[i]=opt->ru[i];
        }
    }
