...
//    {"C"       ,"IQX"       ,""        ,""       ,""       ,""      ,""}, /* SBS */
//    {"IQXDPAN" ,"IQXA"      ,"IQXDPZ"  ,"DPX"    ,"DPX"    ,""      ,""}, /* BDS */
//    {"ABCX"    ,"ABCX"      ,""        ,""       ,""       ,""      ,""}  /* IRN */
//};
/*changed*/
//static char codepris[7][MAXFREQ][16]={  /* code priority for each freq-index */
   /*    0         1          2          3         4         5     */
 //   {"CPYWMNSL","XPYWCMNDLS","IQX"     ,""       ,""       ,""      ,""}, /* GPS */
 //  {"CPABX"   ,"PCABX"     ,"IQX"     ,""       ,""       ,""      ,""}, /* GLO */
//    {"CABXZ"   ,"IQX"       ,"IQX"     ,"ABCXZ"  ,"IQX"    ,""      ,""}, /* GAL */
 //   {"CLSXZ"   ,"LSX"       ,"IQXDPZ"  ,"LSXEZ"  ,""       ,""      ,""}, /* QZS */
 //   {"C"       ,"IQX"       ,""        ,""       ,""       ,""      ,""}, /* SBS */
...
extern int satexclude(int sat, double var, int svh, const prcopt_t *opt)
{
    ...
    prn = sat-NSATGPS-NSATGAL-NSATGLO-NSATQZS;
    ...
    if (sys==SYS_CMP&&(opt->sateph==EPHOPT_SSRAPC||opt->sateph==EPHOPT_SSRCOM)){
        if(prn<=18||prn>=59)
            return 1;
    }
    if (svh) {
...
static int code2freq_GAL(uint8_t code, double *freq)
{
    ...
    switch (obs[0]) {
        ...
        case '5': *freq=FREQ5; return 3; /* E5a */
        case '6': *freq=FREQ6; return 2; /* E6 */
...
static int code2freq_BDS(uint8_t code, double *freq)
{
    char *obs=code2obs(code);
    switch (obs[0]) {
        case '1': *freq=FREQ1;     return 1; /* B1C */
        case '2': *freq=FREQ1_CMP; return 1; /* B1I */
        case '7': *freq=FREQ2_CMP; return 2; /* B2I/B2b */
        case '5': *freq=FREQ5;     return 3; /* B2a */
        case '6': *freq=FREQ3_CMP; return 0; /* B3 */
...
static int readantex(const char *file, pcvs_t *pcvs)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (strstr(buff+60,"TYPE / SERIAL NO")) {
            ...
            if (freq<1||NFREQ<freq||nsys<1) continue;
            if (decodef(buff,3,neu)<3) continue;
            pcv.off[nsys-1][freq-1][0]=neu[pcv.sat?0:1]; /* x or e */
            pcv.off[nsys-1][freq-1][1]=neu[pcv.sat?1:0]; /* y or n */
            pcv.off[nsys-1][freq-1][2]=neu[2];           /* z or u */
        }
        //delete PCV by zrz 2024.4.27
...
//    } //change by zrz
    ...
    WideCharToMultiByte(CP_UTF8, 0, file.cFileName, -1, filename, MAX_PATH, NULL, NULL);
    sprintf(paths[n++],"%s%s",dir,filename);
...
extern double geodist(const double *rs, const double *rr, double *e)
{
    ...
    double oldM_GM = 3.986005E14;
    gravity_delay =  -(2*oldM_GM/(CLIGHT*CLIGHT))*log((Rs+Rr+R)/(Rs+Rr-R));
    ...
    r=norm(e,3);
    for (i=0;i<3;i++) e[i]/=r;
    double temp = OMGE*(rs[0]*rr[1]-rs[1]*rr[0])/CLIGHT ;
    //if(Rr==0)
        return r+OMGE*(rs[0]*rr[1]-rs[1]*rr[0])/CLIGHT ;
    //else
        //return r+OMGE*(rs[0]*rr[1]-rs[1]*rr[0])/CLIGHT - gravity_delay;
}
                     ...
                     int opt, double *dant)
{
    ...
    if(sys==SYS_GPS){
        ...
        m = 1;// change 2024 2 22,1->0

    }
    else if (sys==SYS_CMP){
        m = 2;//add 2024.4.28
    }
    memcpy(tempdel,del,3*sizeof(double));
    if (pcv->off[m][0][1]==0&&pcv->off[m][0][1]==0&&pcv->off[m][0][2]==0){
        m = 0; //add 2024.4.28
    }
    for (i=0;i<NFREQ;i++) {
        for (j=0;j<3;j++) off[j]=pcv->off[m][i][j]+del[j];
